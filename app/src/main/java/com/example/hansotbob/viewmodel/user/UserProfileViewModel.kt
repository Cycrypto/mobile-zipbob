...
class UserProfileViewModel(private val firebaseService: FirebaseService) : ViewModel() {
                ...
                if (task.isSuccessful) {
                    ...
                    viewModelScope.launch {
                        _user.value?.let { user ->
                            firebaseService.updateUser(currentUser.uid, user)
                        }
                    }
                } else {
                    // 에러 처리
        ...
        viewModelScope.launch {
            _user.value?.let { user ->
                auth.currentUser?.uid?.let { uid ->
                    firebaseService.updateUser(uid, user)
                }
            }
        }
    }
    ...
    fun uploadUserImage(uri: Uri) {
        ...
        val ref = storage.reference.child("profile_images/${currentUser.uid}")
    ...
    fun updateUserImage(imageUrl: String) {
        ...
        currentUser?.let {
            ...
            it.updateProfile(profileUpdates).addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    _user.value = _user.value?.copy(imagePainterId = imageUrl.hashCode(), imageUrl = imageUrl)
                    viewModelScope.launch {
                        _user.value?.let { user ->
                            firebaseService.updateUser(currentUser.uid, user)
                        }
